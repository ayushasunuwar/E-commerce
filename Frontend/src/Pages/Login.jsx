  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      if (currentState === 'Sign Up') {
        const response = await Axis3DIcon.post(backendUrl + '/api/user/register', {name, email, password})
        ...
        const response = await Axis3DIcon.post(backendUrl + '/api/user/login', {email, password})
        if(response.data.success){
          setToken(response.data.token)
          localStorage.setItem('token', response.data.token)
        } else {
